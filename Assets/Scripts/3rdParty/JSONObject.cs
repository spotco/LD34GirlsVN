...
public class JSONArray : IEnumerable<JSONValue> {
	...
	public void Remove(int index) {
		if (index >= 0 && index < values.Count) {
			...
			IOut.LogError("index out of range: " + index + " (Expected 0 <= index < " + values.Count + ")");
		}
	}
	...
	public static JSONObject Parse(string jsonString) {
		...
		for (var startPosition = 0; startPosition < jsonString.Length; ++startPosition) {
			...
			case JSONParsingState.String:
				...
				default:
					IOut.LogError("Fatal error, current JSON value not valid");
					return null;
				}
		...
		IOut.LogError("Unexpected end of string");
		return null;
	}
