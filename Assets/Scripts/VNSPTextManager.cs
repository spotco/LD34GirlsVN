...
public class VNSPTextManager : MonoBehaviour {
	...
	private VNSPTextManager i_cons() {
		...
		_sptext = SPText.cons_text(RTex.OSAKA_FNT, RFnt.OSAKA, SPText.SPTextStyle.cons(new Vector4(0,0,0,1), new Vector4(1,1,1,1), new Vector4(1,0,0,1), 0, 0));
		_sptext.set_scale(0.02f);
		_sptext.gameObject.transform.parent = this.transform;
		_sptext.set_u_pos(-9.5f,4.25f);
		_sptext.set_u_z(1);
		...
		_render_tex = new RenderTexture(2048,1024,32);
